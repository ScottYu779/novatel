...
double Novatel::x_zero = 0.0;
double Novatel::y_zero = 0.0;
double Novatel::z_zero = 49.7905;
double Novatel::latitude_zero = 34.399902777777775;
...
bool Novatel::Connect_(std::string port, int baudrate = 115200)
{
    ...
    // serial_port_->write("log bestutmb ontime 1\r\n");
    // serial_port_->write("log bestposb ontime 1\r\n");
    // serial_port_->write("log inspvasb ontime 1\r\n");
    ...
    for (std::vector<std::string>::iterator it = logs.begin(); it != logs.end(); ++it)
    {
        ...
        while (ii < 5)
        {
            try
            {
                ...
                std::stringstream cmd;
                cmd << "LOG " << *it << "\r\n";
                //stringstream.str() return string
                //string.c_str() return char*
                log_info_(cmd.str());
                // wait for acknowledgement (or 2 seconds)
                boost::mutex::scoped_lock lock(ack_mutex_);
                boost::system_time const timeout = boost::get_system_time() + boost::posix_time::milliseconds(2000);
...
void Novatel::ConfigureBaudRate(std::string com_port, int baudrate)
{
    try
    {
        ...
        if (ack_condition_.timed_wait(lock, timeout))
        {
            ...
            log_out << "Ack received.  Baud rate on com port " << com_port << " set to " << baudrate << std::endl;
...
void Novatel::StartReading()
{
    ...
    read_thread_ptr_ = boost::shared_ptr<boost::thread>(new boost::thread(boost::bind(&Novatel::ReadSerialPort, this)));
}
    ...
    while (reading_status_)
    {
        ...
        if (len != 0)
        {
                ...
                (strstr((const char *)buffer, "INSPVAA") != NULL)))
            {
                ...
                if ((team_debug_min_ <= CODE_STATE) && (CODE_STATE <= team_debug_max_))
                {
                    if (!NovatelNode::track_file_out_2_.is_open())
                    {
                ...
                {
                    if (!NovatelNode::track_file_out_1_.is_open())
                    {
...
//see doc:銆奜EM7_Commands_Logs_Manual.pdf銆媝age.50
// header:aa 44 12
void Novatel::BufferIncomingData(unsigned char *message, unsigned int length)
{
    ...
    for (unsigned int ii = 0; ii < length; ii++)
    {
        ...
        { // number of bytes in header - not including sync
            if ((message[ii] == NOVATEL_RESET_BYTE_4) && (message[ii + 2] == NOVATEL_RESET_BYTE_6) && reading_reset_complete_ && waiting_for_reset_complete_)
            {
        ...
        {
            data_buffer_[buffer_index_++] = message[ii];
            //The length in bytes of the body of the message(not including the CRC),and CRC is 4Bytes
...
bool Novatel::ConvertLLaUTM(double Lat, double Long, double *northing, double *easting, int *zone, bool *north)
{
    ...
    *zone = int((LongTemp + 180) / 6.0) + 1;
    ...
    M = a * ((1 - ee / 4 - 3 * ee * ee / 64 - 5 * ee * ee * ee / 256) * LatRad - (3 * ee / 8 + 3 * ee * ee / 32 + 45 * ee * ee * ee / 1024) * sin(2 * LatRad) + (15 * ee * ee / 256 + 45 * ee * ee * ee / 1024) * sin(4 * LatRad) - (35 * ee * ee * ee / 3072) * sin(6 * LatRad));

    *easting = (double)(k0 * N * (A + (1 - T + C) * A * A * A / 6 + (5 - 18 * T + T * T + 72 * C - 58 * e2) * A * A * A * A * A / 120) + 500000.0);
    *northing = (double)(k0 * (M + N * tan(LatRad) * (A * A / 2 + (5 - T + 9 * C + 4 * C * C) * A * A * A * A / 24 + (61 - 58 * T + T * T + 600 * C - 330 * e2) * A * A * A * A * A * A / 720)));
