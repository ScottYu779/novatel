                              ...
                              double &timestamp)
{
  ...
  for (int32_t kk = 0; kk < range.number_of_observations; ++kk)
  {
...
int main(int argc, char *argv[])
{
  ...
  // Find file size
  file1.seekg(0, file1.end);   //set the position at end
  size_t size = file1.tellg(); //get the end lenth

  // Put data into buffer
  unsigned char buff[size];

  file1.seekg(0, file1.beg); //go back to beginning
  ...
  rx1.ConvertLLaUTM(Novatel::latitude_zero, Novatel::longitude_zero, &Novatel::y_zero, 
        &Novatel::x_zero, &Novatel::zoneNum, &Novatel::north);
